...
int main(int argc , char **argv)
{
	...
	nh.getParam("laser_frame_id", laser_frame_id);
	nh.getParam("scan_topic", scan_topic);

	ROS_INFO("laser_frame_id: %s", laser_frame_id.c_str());
	...
	ros::Publisher lidar_pub = nh.advertise<sensor_msgs::LaserScan>(scan_topic, 1); /*create a ROS topic */
